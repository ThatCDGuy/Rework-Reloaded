...
ds_map_set(collision_function, obj_baddie, function(obj) {
	...
	if (obj.object_index == obj_fakepepboss) {
		with (obj) {
			if (
				...
				&& (
					state == states.walk
						|| (state == states.jump && sprite_index == spr_fakepeppino_bodyslamstart)
						|| (state == states.freefall && sprite_index == spr_fakepeppino_bodyslamland)
						|| (state == states.mach2 && attackspeed < 18)
						|| state == states.Sjumpprep
						|| (state == states.throwing && sprite_index != spr_fakepeppino_flailing)
				)
			) {
	...
	if (instance_exists(obj) && obj.invtime == 0 && obj.state != states.grabbed && (!obj.invincible) && obj.instantkillable) {
		if (
			room == boss_pizzaface
			&& (
				obj.object_index == obj_pepperman || obj.object_index == obj_vigilanteboss || obj.object_index == obj_noiseboss || obj.object_index == obj_fakepepboss || obj.object_index == obj_pizzafaceboss_p3
			)
		) {
