...
if (
	...
	&& !ispeppino
	&& !skateboarding
	&& ((scr_check_superjump() && key_jump2) || key_superjump)
	&& state != states.mach3
	&& can_jump
	&& vsp > 0
	&& (state == states.normal || state == states.mach2)
) {
...
if (
	...
	&& prevstate == states.machcancel
	&& sprite_index != spr_playerN_divebomb
	&& sprite_index != spr_playerN_divebombfall
	&& sprite_index != spr_playerN_divebombland
	&& (prevsprite == spr_playerN_divebomb || prevsprite == spr_playerN_divebombfall || prevsprite == spr_playerN_divebombland)
) {
...
if (
	...
	&& state != states.Sjump
	&& state != states.tumble
	&& !instance_exists(obj_vigilante_duelintro)
) {
...
if (blur_effect > 0) {
	...
	breakdance_speed >= 0.6
	|| (state == states.slipbanan && sprite_index == spr_rockethitwall)
	|| mach4mode == true
	|| boxxeddash == true
	|| state == states.ghost
	|| state == states.tumble
	...
	|| (
		state == states.barrelslide
			|| (state == states.grab && sprite_index == spr_swingding && swingdingdash <= 0)
...
if (
	state != states.freefall
	&& state != states.superslam
	&& (state != states.chainsaw || (tauntstoredstate != states.freefall && tauntstoredstate != states.superslam))
	&& (state != states.backbreaker || (tauntstoredstate != states.freefall && tauntstoredstate != states.superslam))
	...
	if (
		!(
			state == states.door
				...
				|| (state == states.knightpep && (sprite_index == spr_knightpepstart || sprite_index == spr_knightpepthunder))
...
if (
	state == states.punch
	|| (state == states.jump && sprite_index == spr_playerN_noisebombspinjump)
...
if (
	(state == states.ratmountbounce && vsp >= 0)
	|| (state == states.noisecrusher && vsp >= 0)
	...
	|| (state == states.shotgunfreefall && (sprite_index == spr_shotgunjump2 || sprite_index == spr_shotgunjump3))
	|| state == states.Sjump
	|| state == states.rocket
	|| state == states.rocketslide
	|| state == states.chainsawbump
	|| (state == states.punch && ((sprite_index != spr_breakdanceuppercut && sprite_index != spr_breakdanceuppercutend) || vsp < 0))
	...
	|| (state == states.firemouth && sprite_index != spr_firemouthintro)
	...
	|| state == states.machfreefall
	|| state == states.tacklecharge
	|| (state == states.superslam && sprite_index == spr_piledriver)
	|| state == states.knightpep
	|| state == states.knightpepattack
	|| state == states.knightpepslopes
	|| state == states.trickjump
	...
	|| (global.noisejetpack == true && (ispeppino || noisepizzapepper))
	...
	|| (!ispeppino && state == states.slipnslide && instance_exists(obj_surfback))
	|| ghostdash == true
	|| state == states.slipbanan
	|| state == states.shoulderbash
	|| (state == states.machslide && (sprite_index == spr_mach3boost || sprite_index == spr_player_machslideboost3fall))
) {
	instakillmove = true;
} else {
	instakillmove = false;
...
if (
	(global.noisejetpack || holycross > 0)
	&& (
		state == states.actor
			|| state == states.chainsaw
...
if (
	...
	&& !(state == states.mach2)
	&& ratmount_movespeed < 12
	&& (state != states.ghost || ghostpepper < 2 || !ghostdash)
	&& gusdashpadbuffer <= 0
	&& state != states.Sjump
) {
	macheffect = false;
...
if (toomuchalarm1 > 0) {
	...
	if (
		toomuchalarm1 <= 0
		&& !instance_exists(obj_swapgusfightball)
		&& (state == states.mach3 || (state == states.ghost && ghostdash && ghostpepper >= 3) || state == states.mach2 || state == states.Sjump || ratmount_movespeed >= 12 || gusdashpadbuffer > 0)
	) {
		with (create_mach3effect(x, y, sprite_index, image_index - 1)) {
			playerid = other.object_index;
			image_xscale = other.xscale;
...
if (
	(y > (room_height + 300) || y < -800)
	&& !place_meeting(x, y, obj_verticalhallway)
	&& restartbuffer <= 0
	&& !verticalhallway
	&& state != states.dead
	&& state != states.gotoplayer
	...
	&& state != states.dead
	&& !instance_exists(obj_backtohub_fadeout)
...
if (character != "M") {
	if (!scr_solid_player(x, y)) {
		if (
			...
			&& state != states.boxxedpepspin
			&& !(state == states.bump && sprite_index == spr_tumbleend)
			&& (state != states.barrelslide && state != states.barrelclimbwall)
			...
			&& (state != states.pistol && sprite_index != spr_player_crouchshoot)
			&& state != states.Sjumpprep
