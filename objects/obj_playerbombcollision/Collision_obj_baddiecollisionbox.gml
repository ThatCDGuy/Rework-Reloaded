...
if ((!baddie.invincible) || baddie.object_index == obj_vigilanteboss || baddie.object_index == obj_fakepepboss || baddie.object_index == obj_pizzafaceboss_p2) {
	...
	(baddie.object_index != obj_pizzafaceboss || ((baddie.state != states.pizzaface_ram || baddie.substate != states.jump) && baddie.state != states.walk))
	&& (baddie.object_index != obj_pepperman || baddie.state != states.mini)
) {
	with (bombID) {
		deadbuffer--;
	}
