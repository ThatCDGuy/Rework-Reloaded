...
var categories = create_menu_fixed(menu_options.options, anchor.center, 0, 48, -4);
add_option_press(categories, 0, "option_audio", function() {
	menu_goto(menu_options.audio);
});
add_option_press(categories, 1, "option_video", function() {
	menu_goto(menu_options.video);
});
add_option_press(categories, 2, "option_game", function() {
	menu_goto(menu_options.game);
});
add_option_press(categories, 3, "option_controls", function() {
	...
	*/
	menu_goto(menu_options.controls);
});
...
var audio_menu = create_menu_fixed(menu_options.audio, anchor.left, 150, 40);
add_option_press(audio_menu, 0, "option_back", function() {
	menu_goto(menu_options.options);
...
var window_menu = create_menu_fixed(menu_options.window, anchor.left, 150, 40, menu_options.video);
...
var resolution_menu = create_menu_fixed(menu_options.resolution, anchor.left, 150, 40, menu_options.video);
...
var game_menu = create_menu_fixed(menu_options.game, anchor.left, 150, 40);
...
add_option_multiple(game_menu, 4, "option_timer_type", [
	create_option_value("option_timer_level", 0),
	create_option_value("option_timer_save", 1),
	create_option_value("option_timer_levelsave", 2)
], function(val) {
	ini_open_from_string(obj_savesystem.ini_str_options);
...
add_option_press(controls_menu, 3, "option_reset_config", function() {
	...
	with (obj_option) {
		for (var i = 0; i < array_length(menus); i++) {
			var b = menus[i];
			if (b.menu_id == menu_options.controller || b.menu_id == menu_options.deadzone || b.menu_id == menu_options.keyboard) {
...
var keyboard_menu = create_menu_fixed(menu_options.keyboard, anchor.left, 150, 40, menu_options.controls);
...
var controller_menu = create_menu_fixed(menu_options.controller, anchor.left, 150, 40, menu_options.controls);
...
add_option_press(controller_menu, 2, "option_deadzone_title", function(val) {
	menu_goto(menu_options.deadzone);
});
...
var back = menu_options.controller;
var deadzones_menu = create_menu_fixed(menu_options.deadzone, anchor.left, 150, 40, back);
