...
add_achievement_update(
	...
	function() {
		...
		if (
			obj_player1.grounded
			|| (
				obj_player1.state != states.ratmountbounce && obj_player1.state != states.chainsaw && obj_player1.sprite_index != spr_playerN_sidewayspin && obj_player1.sprite_index != spr_playerN_sidewayspinend
			)
		) {
			_count.value = 0;
		}
...
add_achievement_notify(
	...
	function(data) {
		...
		if (type == notifs.cancel_noisedrill) {
			...
			type == notifs.baddie_kill
			&& (obj_player1.sprite_index == spr_playerN_divebomb || obj_player1.sprite_index == spr_playerN_divebombfall || obj_player1.sprite_index == spr_playerN_divebombland)
		) {
			count.value++;
			if (count.value >= 4) {
...
var pats = [["critic", 7, boss_pepperman], ["outlaw", 8, boss_vigilante], ["antidoise", 9, boss_noise], ["flesheater", 10, boss_fakepep], ["super", 11, boss_pizzafacehub]];
for (i = 0; i < array_length(pats); i++) {
	pat = pats[i];
	p = add_achievement_notify(
		concat("pal_", pat[0]),
		function() {
...
add_achievement_notify(
	...
	function(data) {
		...
		if (
			type == notifs.baddie_kill
			&& global.leveltosave == "medieval"
			&& (obj_player1.state == states.tumble || (obj_player1.tauntstoredstate == states.tumble && obj_player1.state == states.chainsaw))
			&& (obj_player1.sprite_index == obj_player1.spr_tumblestart || obj_player1.sprite_index == obj_player1.spr_tumbleend || obj_player1.sprite_index == obj_player1.spr_tumble)
		) {
