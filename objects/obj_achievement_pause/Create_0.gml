...
if (lvl != "pepperman" && lvl != "vigilante" && lvl != "pizzaface" && lvl != "fakepep" && lvl != "noise") {
	...
	for (var i = 0; i < sprite_get_number(sprite) / 2; i++) {
		...
		array_push(achievements, {name: lang_get_value(concat(n, "title")), description: lang_get_value_newline(n), got: _got});
	}
} else {
	...
	array_push(achievements, {name: lang_get_value(concat(n, "title")), description: lang_get_value_newline(n), image_index: ix, got: _got});
}
ini_close();
