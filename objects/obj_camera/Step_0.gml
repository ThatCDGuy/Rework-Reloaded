	...
	switch (state) {
		case states.normal:
			...
			if (target.cutscene || (target.collision_flags & colflag.secret) > 0) {
				...
				(abs(target.hsp) >= 16 || (target.state == states.chainsaw && target.tauntstoredmovespeed >= 16))
				&& player.state != states.climbwall
				&& player.state != states.Sjump
				&& player.state != states.ghost
			) {
