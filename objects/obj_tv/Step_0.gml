...
if (
	...
	|| (string_copy(room_get_name(room), 1, 5) == "tower" && !global.panic)
) {
	visible = false;
		...
		switch (expressionsprite) {
			...
			case spr_tv_exprcomboN:
				if (
					...
					|| obj_player1.mach4mode
					|| obj_player1.state == states.hurt
					|| obj_player1.state == states.mach3
					|| obj_player1.sprite_index == obj_player1.spr_mach3boost
			...
			case spr_tv_exprmach3N:
				with (obj_player1) {
					if (
						...
						&& (state != states.chainsaw || (tauntstoredstate != states.mach3 && tauntstoredstate != states.climbwall))
