...
switch (state) {
	case states.titlescreen:
		...
		if (!instance_exists(obj_noiseunlocked)) {
			jumpscarecount++;
			if (
				(keyboard_check_pressed(vk_anykey) || scr_checkanygamepad(obj_inputAssigner.player_input_device[0]) != -4 || scr_checkanystick(obj_inputAssigner.player_input_device[0]))
