...
function boss_hurt_gustavo() {
	if (pizzahead) {
		with (obj_gustavograbbable) {
			...
			if (
				(thrown || _slam)
				&& other.state != states.KO
				&& (place_meeting(x + hsp, y, other) || place_meeting(x - image_xscale, y, other) || place_meeting(x - (32 * image_xscale), y, other) || place_meeting(x + (32 * image_xscale), y, other))
			) {
				with (other) {
					state = states.stun;
					stunned = 1000;
