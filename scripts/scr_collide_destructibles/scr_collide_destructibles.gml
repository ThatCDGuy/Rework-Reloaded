function scr_collide_destructibles() {
	with (obj_player) {
		if (
			...
			|| (ghostdash && sprite_index != spr_ghostidle)
		...
		if (
			...
			|| (state == states.boxxedpepjump && boxxeddash)
			|| (state == states.boxxedpep && boxxeddash)
		...
		if (
			(
				state == states.knightpep
					...
					|| state == states.hookshot
					|| (state == states.bombpepup && bombup_dir == 1)
			)
			&& vsp > 0
		) {
			var vy = 1;
