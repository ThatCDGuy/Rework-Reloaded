function scr_hurtplayer(player) {
	...
	var _swap = false;
	with (player) {
		if (global.failcutscene) {} else if (
			state == states.ratmounthurt
			|| state == states.duel
			|| state == states.supergrab
			|| state == states.pizzaface_phase2transition
			|| state == states.parry
		...
		) {} else if (global.noisejetpack == 1 && (ispeppino || noisepizzapepper)) {} else if (holycross > 0 || invtime > 0) {} else if (sprite_index == spr_jetpackstart2) {} else if (
			(
				state == states.backbreaker
					&& (
						parrytimer > 0
							|| instance_exists(obj_parryhitbox)
							|| sprite_index == spr_supertaunt1
							|| sprite_index == spr_supertaunt2
							|| sprite_index == spr_supertaunt3
		...
		} else if (instance_exists(obj_pizzafaceboss) && obj_pizzafaceboss.state == states.transition) {} else if (instance_exists(obj_pizzafaceboss_p2) && obj_pizzafaceboss_p2.state == states.fall) {} else if (
			state == states.shotgundash
		) {} else if ((state == states.knightpep || state == states.knightpepattack || state == states.knightpepslopes || state == states.knightpepbump) && cutscene == 0) {} else if (
			state == states.ghost
		...
		} else if (state == states.slipnslide) {} else if (state == states.trickjump || state == states.chainsaw) {} else if (state == states.chainsawbump) {} else if (
			state == states.bombpep
			&& hurted == 0
		) {} else if (state == states.rideweenie) {} else if (state == states.slipnslide) {} else if (pizzashield == 1) {
			pizzashield = false;
			...
			&& (hurted == 0 || state == states.cheesepep || state == states.cheesepepstickside || state == states.cheesepepstickup)
			&& cutscene == 0
		) {
		...
		if (_hurt) {
			...
			if (!_swap) {
				...
				with (obj_tv) {
					...
					if (
						(state == states.tv_expression || state == states.tv_whitenoise)
						&& (sprite_index == spr_tv_exprhurt || sprite_index == spr_tv_exprhurtN || sprite_index == spr_tv_hurtG || str2 == "spr_tv_exprhurt" || str2 == "spr_tv_exprhurtN")
					) {
			...
			with (obj_bosscontroller) {
				if (!instance_exists(obj_hpeffect)) {
					...
					scr_bosscontroller_particle_hp(
						spr_bossfight_playerhp,
						irandom(sprite_get_number(spr_bossfight_playerhp) - 1),
						d.x,
						d.y,
						(d.x > (room_width / 2)) ? -1 : 1,
						d.spr_palette,
						d.paletteselect,
						d.patterntexture
					);
