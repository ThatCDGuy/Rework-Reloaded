...
function scr_do_rank(showtoppins = true, boss = false) {
	...
	if (global.combo > 0) {
		...
		for (var i = 0; i < global.comboscore; i += 10) {
			create_collect(
				obj_player1.x + irandom_range(-60, 60),
				(obj_player1.y - 100) + irandom_range(-60, 60),
				choose(spr_shroomcollect, spr_tomatocollect, spr_cheesecollect, spr_sausagecollect, spr_pineapplecollect),
				10
			);
		}
		global.comboscore = 0;
