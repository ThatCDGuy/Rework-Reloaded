...
function scr_player_ghost() {
	...
	if (!ispeppino) {
		...
		if (!ghostdash) {
			...
			if (
				scr_solid_player((x + sign(hsp)), (y + sign(vsp)))
				&& !place_meeting((x + sign(hsp)), (y + sign(vsp)), obj_destructibles)
				&& (ghostpepper < 3 || !place_meeting((x + sign(hsp)), (y + sign(vsp)), obj_metalblock))
				&& !place_meeting((x + sign(hsp)), (y + sign(vsp)), obj_slope)
			) {
				ghostbumpbuffer++;
