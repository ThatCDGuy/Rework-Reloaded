...
function scr_add_grannypizzalevel(level, _icon, _secrets = true, _toppins = true, _treasure = true, _rank = true) {
	var q = {icon: _icon, secrets: _secrets, secretcount: 0, toppins: _toppins, toppinarr: [false, false, false, false, false], treasure: _treasure, gottreasure: false, rank: _rank, gotrank: -4};
	ini_open_from_string(obj_savesystem.ini_str);
	if (q.secrets) {
