function scr_player_ratmountgroundpound() {
	...
	if (sprite_index == spr_lonegustavo_groundpound || sprite_index == spr_lonegustavo_groundpoundstart) {
		...
		if (
			grounded
			&& vsp > 0
			&& !place_meeting(x, y + vsp + 2, obj_grindrail)
			&& !place_meeting(x, y + 10, obj_grindrail)
			&& !place_meeting(x, y + vsp, obj_destructibles)
			&& !place_meeting(x, y + vsp + 2, obj_destructibles)
			&& !place_meeting(x, y + 10, obj_destructibles)
		) {
			if (scr_slope()) {
