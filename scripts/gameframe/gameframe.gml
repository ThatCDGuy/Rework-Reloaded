...
// Feather disable all
#region prototypes
globalvar mq_game_frame_button;
mq_game_frame_button = [undefined, /* 1:name */ undefined, /* 2:custom */ undefined, /* 3:icon */ undefined, /* 4:subimg */ 0, /* 5:margin_left */ 0, /* 6:margin_right */ 0, /* 7:hover */ undefined, /* 8:pressed */ undefined, /* 9:enabled */ undefined, /* 10:fade */ 0, /* 11:click */ undefined, /* 12:get_width */ 0, /* 13:update */ undefined, /* 14:draw_underlay */ undefined, /* 15:draw_icon */ undefined];
globalvar mq_gameframe_delayed_item;
mq_gameframe_delayed_item = [undefined, /* 1:func */ undefined, /* 2:time */ 0, /* 3:arg0 */ undefined, /* 4:arg1 */ undefined, /* 5:arg2 */ undefined, /* 6:arg3 */ undefined];
#endregion
#region metatype
globalvar gameframe_std_haxe_type_markerValue;
gameframe_std_haxe_type_markerValue = [];
...
function gameframe_update() {
	...
	if (!gameframe_isMaximized_hx && gameframe_has_native_extension && gameframe_delayed_frame_index > 3 && !gameframe_get_shadow()) {
	...
	if (gameframe_drag_flags == 0) {
		...
		if (gameframe_can_input && gameframe_can_resize) {
			switch (__flags) {
				case 1:
				case 4:
					__cursor = cr_size_we;
				...
				case 3:
				case 12:
					__cursor = cr_size_nwse;
					break;
				case 6:
				case 9:
					__cursor = cr_size_nesw;
					break;
			}
		}
...
function game_frame_button_draw_icon_default(_b, _x, _y, _width, _height) {
	...
	draw_sprite_ext(
		_icon,
		_b[4 /* subimg */],
		(_x + ((_width - sprite_get_width(_icon) * _scale) div 2) + (sprite_get_xoffset(_icon) * _scale | 0)),
		_y + ((_height - sprite_get_height(_icon) * _scale) div 2) + (sprite_get_yoffset(_icon) * _scale | 0),
		_scale,
		_scale,
		0,
		gameframe_blend,
		gameframe_alpha * (_b[9 /* enabled */] ? 1 : 0.3)
	);
}
...
function gameframe_button_update(_x, _y, _height, _mx, _my) {
	...
	if (gameframe_button_wait_for_movement) {
		if (_mx != gameframe_button_wait_for_movement_x || _my != gameframe_button_wait_for_movement_y) {
...
function gameframe_store_rect() {
	...
	gameframe_tools_rect_get_window_rect(gameframe_restoreRect_hx);
	if (gameframe_debug) {
		gameframe_log("storeRect: ", gameframe_restoreRect_hx);
	}
}
...
function gameframe_restore(__force) {
	...
	if (!__force && !gameframe_isMaximized_hx && !gameframe_isFullscreen_hx) {
		exit;
	}
...
function gameframe_set_fullscreen_1(__mode, __wasFullscreen) {
	...
	if (gameframe_debug) {
		gameframe_log("setFullscreen(mode:", __mode, ", wasfs:", __wasFullscreen, ")");
	}
	...
	switch (__mode) {
		...
		case 2:
			...
			if (!gameframe_isMaximized_hx && !__wasFullscreen) {
				gameframe_store_rect();
			}
...
function gameframe_set_window_cursor(_cr1) {
	...
	if (gameframe_set_cursor) {
		if (window_get_cursor() != _cr1) {
			window_set_cursor(_cr1);
		}
	}
}
...
function gameframe_caption_draw_caption_text_default(__x, __y, __width, __height) {
	...
	if (_icon != -1) {
		draw_sprite_ext(
			_icon,
			-1,
			(__x + sprite_get_xoffset(_icon) * _dpiScale | 0),
			__y + ((__height - sprite_get_height(_icon) * _dpiScale) div 2) + sprite_get_yoffset(_icon) * _dpiScale,
			_dpiScale,
			_dpiScale,
			0,
			16777215,
			gameframe_caption_alpha * gameframe_alpha
		);
	...
	draw_text_ext_transformed(
		(__x + ((gameframe_caption_text_align * __textWidth) div 2)),
		__y + ((__height - string_height_ext(_text, -1, __textWidth) * _dpiScale) div 2),
		_text,
		-1,
		__textWidth,
		_dpiScale,
...
function gameframe_cover_ensure() {
	...
	if (!gameframe_tools_rect_equals(gameframe_cover_curr_rect, __target_rect)) {
		...
		if (gameframe_cover_can_ignore && gameframe_tools_rect_equals(__target_rect, gameframe_cover_ignore_rect)) {
			exit;
		}
		if (gameframe_debug) {
...
function gameframe_delayed_call_impl(_func, _delay, _arg0, _arg1, _arg2, _arg3) {
	...
	var _item;
	if (ds_stack_empty(gameframe_delayed_pool)) {
		_item = gameframe_delayed_item_create();
	} else {
		_item = ds_stack_pop(gameframe_delayed_pool);
	}
	_item[@ 1 /* func */] = _func;
...
function gameframe_delayed_update() {
	...
	while (!ds_queue_empty(gameframe_delayed_queue)) {
		...
		if (_head[2 /* time */] > gameframe_delayed_frame_index) {
			break;
		}
	...
	switch (gameframe_drag_flags) {
		...
		case 32:
			if (point_distance(__mx, __my, gameframe_drag_mx, gameframe_drag_my) > 5) {
				...
				if (gameframe_drag_mx - gameframe_drag_left < (gameframe_drag_right - gameframe_drag_left) / 2) {
					__x = min(gameframe_drag_mx - gameframe_drag_left, (gameframe_restoreRect_hx[2 /* width */] >> 1));
				} else {
					__x = max(gameframe_restoreRect_hx[2 /* width */] + gameframe_drag_mx - gameframe_drag_right, (gameframe_restoreRect_hx[2 /* width */] >> 1));
				}
				gameframe_isMaximized_hx = false;
		...
		case 2:
			...
			break;
		case 4:
			window_set_rectangle(gameframe_drag_left, gameframe_drag_top, gameframe_drag_right - gameframe_drag_left - gameframe_drag_mx + __mx, gameframe_drag_bottom - gameframe_drag_top);
			break;
		case 8:
			window_set_rectangle(gameframe_drag_left, gameframe_drag_top, gameframe_drag_right - gameframe_drag_left, gameframe_drag_bottom - gameframe_drag_top - gameframe_drag_my + __my);
			break;
		case 3:
			var __x = __mx - (gameframe_drag_mx - gameframe_drag_left);
			var __y = __my - (gameframe_drag_my - gameframe_drag_top);
		...
		case 12:
			window_set_rectangle(
				gameframe_drag_left,
				gameframe_drag_top,
				gameframe_drag_right - gameframe_drag_left - gameframe_drag_mx + __mx,
				gameframe_drag_bottom - gameframe_drag_top - gameframe_drag_my + __my
			);
			break;
	}
}
...
function gameframe_draw() {
	...
	var __buttons_x = gameframe_button_get_combined_offset(_gw);
	if (!gameframe_isMaximized_hx) {
		gameframe_caption_draw_border(0, 0, _gw, _gh);
	}
...
function gameframe_tools_mon_get_active() {
	...
	for (var __g1 = __count; _i < __g1; _i++) {
		...
		if (__cx1 >= __mntr[0 /* x */] && __cy1 >= __mntr[1 /* y */] && __cx1 < __mntr[0 /* x */] + __mntr[2 /* width */] && __cy1 < __mntr[1 /* y */] + __mntr[3 /* height */]) {
			return __item;
		}
	}
	var __item = __list[| 0];
...
gameframe_button_wait_for_movement_x = 0.0;
globalvar gameframe_button_wait_for_movement_y; /// @is {number}
gameframe_button_wait_for_movement_y = 0.0;
// gameframe:
...
gameframe_dpi_scale = 1.0;
// gameframe:
globalvar gameframe_effective_scale; /// @is {number}
gameframe_effective_scale = 1.0;
