function scr_playersounds() {
	with (obj_player) {
		...
		if (
			(state == states.ratmount || state == states.ratmountjump)
			&& (
		...
		if (!ispeppino) {
			...
			if (
				(sprite_index == spr_playerN_sidewayspin || sprite_index == spr_playerN_sidewayspinend)
				&& (state == states.mach2 || state == states.mach3 || ((tauntstoredstate == states.mach2 || tauntstoredstate == states.mach3) && state == states.chainsaw))
			) {
