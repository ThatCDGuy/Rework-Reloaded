function save_level_update(levelname) {
	...
	if (save_step == -1) {
		...
		struct = {width: _room.width, height: _room.height, x: _room.x, y: _room.y, instances: array_create(0), backgrounds: array_create(0), tiles: array_create(0)};
		for (var i = 0; i < array_length(_room.backgrounds); i++) {
			var b = _room.backgrounds[i];
			var bg = {sprite_index: sprite_get_name(b.sprite_index), depth: b.depth};
		...
		for (i = 0; i < array_length(_room.instances); i++) {
			...
			with (b) {
				var object_struct = {x: b.x, y: b.y, object_index: object_get_name(b.object), ID: b.ID};
