	...
	switch (character) {
		case "P":
			...
			if (
				...
				&& key_attack
				&& key_fightball
				&& !instance_exists(obj_swapmodegrab)
				&& !instance_exists(obj_swapdeatheffect)
				&& !instance_exists(obj_noiseanimatroniceffect)
				&& obj_swapmodefollow.animatronic <= 0
			) {
			...
			if (
				((!key_attack && fightball == 0 && !launched) && sprite_index != spr_dashpadmach && grounded && vsp > 0 && (character == "P" || character == "N"))
				|| ((character == "S" && (move == 0 || move != xscale) && grounded) && fightball == 0)
			) {
			...
			if (
				(
					!grounded
						&& (place_meeting(x + hsp, y, obj_solid) || scr_solid_slope(x + hsp, y))
				...
				|| (
					grounded
						&& (place_meeting(x + sign(hsp), y - 16, obj_solid) || scr_solid_slope(x + sign(hsp), y - 16))
						&& !place_meeting(x + hsp, y, obj_destructibles)
						&& !place_meeting(x + hsp, y, obj_mach3solid)
						&& !place_meeting(x + hsp, y, obj_metalblock)
						&& place_meeting(x, y + 1, obj_slope)
				)
			) {
				var _climb = true;
				if (!ispeppino) {
			...
			if (
				(scr_solid(x + sign(hsp), y) && !place_meeting(x + sign(hsp), y, obj_mach3solid))
				&& !scr_slope()
				...
				&& !place_meeting(x + sign(hsp), y, obj_climbablewall)
				&& grounded
			) {
				var _bump = true;
